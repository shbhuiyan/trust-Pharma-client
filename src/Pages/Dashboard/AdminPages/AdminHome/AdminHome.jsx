...
const AdminHome = () => {
  ...
  const confirm = paymentHistory.filter(
    (confirmPayment) => confirmPayment.status === "confirmed"
  );
  const confirmTotal = confirm.reduce((prev, item) => prev + item.amount, 0);
  const pending = paymentHistory.filter(
    ...
    <section className="my-10 space-y-20 font-cinzel">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        ...
        <div className="p-6 rounded-xl shadow-2xl text-center bg-gradient-to-r from-teal-500">
          ...
          <p className="mt-4 text-3xl font-bold flex items-center gap-4 justify-center">
            <FaTruckFast /> {paymentHistory.length}
          </p>
        </div>
      </div>
